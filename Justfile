...
rechunk $image="bluefin" $tag="latest" $flavor="main" ghcr="0" pipeline="0":
    ...
    if [[ "{{ ghcr }}" == "1" ]]; then
        ...
        ID=$(${SUDOIF} ${PODMAN} images --filter reference=ghcr.io/iitzrohan/"${base_image_name}":${fedora_version} --format "{{ '{{.ID}}' }}")
        if [[ -n "$ID" ]]; then
            ${PODMAN} rmi "$ID"
        fi
    fi
    ...
    ${SUDOIF} ${PODMAN} run --rm \
        ...
        --volume cache_ostree:/var/ostree \
        --env REPO=/var/ostree/repo \
        --env PREV_REF=ghcr.io/iitzrohan/"${image_name}":"${tag}" \
...
build-iso $image="bluefin" $tag="latest" $flavor="main" ghcr="0" pipeline="0":
    ...
    if [[ "{{ ghcr }}" == "1" ]]; then
        IMAGE_FULL=ghcr.io/iitzrohan/"${image_name}":"${tag}"
        IMAGE_REPO=ghcr.io/iitzrohan
    ...
    FLATPAK_DIR_SHORTNAME="silverblue_flatpaks"
